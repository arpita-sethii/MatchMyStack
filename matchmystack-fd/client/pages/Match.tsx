// client/pages/Match.tsx - FINAL CLEANED VERSION
// Removed: Filters, Super-like, Intro, Role fit, My likes
// Kept: Resume uploader, Skills editor, Clean swipe deck
...
const SAMPLE_PROJECTS: ProfileCard[] = [
  { id: "p-101", name: "Auth microservice", role: "Backend", bio: "Auth with JWT, OAuth and short lived refresh tokens.", skills: ["FastAPI", "Postgres", "Docker"] },
  { id: "p-102", name: "Realtime Chat", role: "Realtime / Websockets", bio: "Realtime messaging with presence and typing indicators.", skills: ["Socket.IO", "Redis", "Node"] },
  { id: "p-103", name: "Frontend Dashboard", role: "Frontend", bio: "Beautiful analytics dashboard in React + Tailwind.", skills: ["React", "TypeScript", "D3"] },
];

export default function Match() {
  const [matches, setMatches] = useState<ProfileCard[] | null>(null);
  ...
  const autoMatchFromSkills = async (skills: string[], topK = 10) => {
    ...
    try {
      ...
      console.error("autoMatchFromSkills failed", e);
      return null;
    }
  };
    ...
    (async () => {
      try {
        ...
        if (me?.skills && Array.isArray(me.skills) && me.skills.length > 0) {
          ...
          const res = await autoMatchFromSkills(normalized, 10);
          if (res) {
            handleMatches(res);
  ...
  const saveSkillsToServer = async (skillsToSave?: string[]) => {
    ...
    try {
      const token = localStorage.getItem("mms_token");
      if (!token) {
        setToast({ text: "Not authenticated", type: "err" });
        return;
      }
      ...
      if (!resp.ok) {
        ...
        console.error("Save skills error:", errorText);
        setToast({ text: `Save failed: ${resp.status}`, type: "err" });
        return;
      }

      setToast({ text: "Skills saved! Upload a resume to refresh matches.", type: "ok" });
      try { await refreshUser(); } catch {}
    } catch (e: any) {
      console.error("saveSkillsToServer failed", e);
      setToast({ text: `Save failed: ${e.message}`, type: "err" });
    }
  };
    ...
    try {
      ...
      if (action === "match") {
        try {
          const token = localStorage.getItem("mms_token");
          if (token) {
            const chatResponse = await fetch(`${API_BASE}/chat/rooms/${projectId}`, {
              method: "POST",
              headers: { 
                "Authorization": `Bearer ${token}`,
                "Content-Type": "application/json"
  ...
  const displayMatches = matches ?? SAMPLE_PROJECTS;
    ...
    <div className="mx-auto max-w-6xl">
      ...
      <div className="flex items-start gap-8">
        <div className="w-full md:w-2/3 space-y-6">
          ...
          {/* 3) Swipe deck - ✅ CLEANED: No filters, no super-like, no intro */}
          <div className="rounded-xl border p-6">
            <div className="mb-6">
